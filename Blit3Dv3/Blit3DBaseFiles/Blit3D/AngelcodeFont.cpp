...
AngelcodeFont::AngelcodeFont(std::string fontfile, TextureManager *TexManager, GLSLProgram *shader)
{
	...
	while (current < fsize - 1)
	{
		...
		case 2: //common block
		{
			...
			lineHeight = (float)ReadShortAndAdvance(marker, buffer) * 0.8f;
...
void AngelcodeFont::BlitText(float x, float y, std::string output)
{
	...
	prog->setUniform("in_Scale_X", scale_x); //default scaling
	prog->setUniform("in_Scale_Y", scale_y); //default scaling
	...
	for (unsigned int i = 0; i < output.size(); ++i)
	{
		...
		if (itr != Chars.end())
		{
			...
			{
				//Multiply itrK->second by scale_x
				modelMatrix = glm::translate(modelMatrix, glm::vec3(itrK->second * scale_x, 0.f, 0.f));
			}

			// draw a quad: 1 quad x 4points per quad = 4 verts, the third argument
			glDrawArrays(GL_QUADS, itr->second.lookupVerts * 4, 4); // This line here is what I need to look at.
			modelMatrix = glm::translate(modelMatrix, glm::vec3(itr->second.xAdvance * scale_x, 0.f, 0.f)); //Here
			prog->setUniform("modelMatrix", modelMatrix);
...
void AngelcodeFont::BlitText(float x, float y, float scale_val_x, float scale_val_y, std::string text)
{
	scale_x = scale_val_x;
	scale_y = scale_val_y;
...
void AngelcodeFont::BlitAutoText(float x, float y, std::string text)
{
	float halfWidth = WidthText(text) / 2;
	float halfHeight = FontHeight() / 2;

	BlitText(x - halfWidth, y + halfHeight, text);
}

void AngelcodeFont::BlitAutoText(float x, float y, float scale_val_x, float scale_val_y, std::string text)
{
	float halfWidth = WidthText(text) / 2 * scale_val_x;
	float halfHeight = FontHeight() / 2;
...
//Returns the height of the text font, in pixels
float AngelcodeFont::FontHeight()
{
	//float height_font = lineHeight * 0.8f;
